...
from pipeline.b_silver.transform import prepare_movie_df, prepare_users_df, prepare_ratings_df
...
@patch("pipeline.b_silver.transform.initialize_s3_client")
def test_prepare_movie_df_write_failure(mock_init_client, mock_write_file, user_data, caplog):
    """
    Integration test for prepare_movie_df() when reading from bronze fails.
...
def test_prepare_ratings_df_success(mock_init_client, rating_data):
    ...
    with patch("config.S3_BUCKET_BRONZE", "movie-pipeline-bronze"), \
        ...
        kwargs = {"data_interval_start": pd.Timestamp("1999-12-25", tz="UTC"),
        ...
        # Load the CSV from mock S3 directly into a DataFrame
        response = s3.get_object(Bucket="movie-pipeline-silver", Key="ratings/2000-01.csv")
        df_result = pd.read_csv(io.BytesIO(response["Body"].read()))
...
@patch("pipeline.b_silver.transform.initialize_s3_client")
def test_prepare_ratings_df_write_failure(mock_init_client, mock_watermarks, rating_data, s3_mock_env, caplog):
    """
    Integration test for prepare_movie_df() when reading from bronze fails.
