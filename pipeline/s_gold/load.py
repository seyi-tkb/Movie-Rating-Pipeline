...
def check_tables():
    ...
    # ensure partitioning functions exist for ratings table
    try:
...
def load_ratings_df(pipeline_start, **kwargs: dict):
    ...
    try:
        ...
        logger.info(f"Monthly partition ensured in prod.ratings.")
    except Exception as e:
        logger.error(f"Partition creation failed: {str(e)}", exc_info=True)
