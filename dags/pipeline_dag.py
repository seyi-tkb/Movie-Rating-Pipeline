...
from utils.freshness_checks import check_movies_file, check_ratings_file, check_users_file
...
with DAG('Rating Pipeline',
    ...
    # Bronze tasks
    # -------------------
    to_bronze = PythonOperator(task_id = 'to_bronze',
                               python_callable = source_to_bronze)
    
    # -------------------
    # Silver tasks
    # -------------------
    check_movie_freshness = ShortCircuitOperator(task_id="check_movie_freshness",
                                     python_callable=check_movies_file)
    
    check_users_freshness = ShortCircuitOperator(task_id="check_users_freshness",
                                     python_callable=check_users_file)
    
    check_ratings_freshness = ShortCircuitOperator(task_id="check_ratings_freshness",
                                     python_callable=check_ratings_file)
    

    movies_to_silver = PythonOperator(task_id= "load_movies",
                                 python_callable= prepare_movie_df,
                                 op_kwargs= {'pipeline_start': pipeline_start})
    
    users_to_silver = PythonOperator(task_id= "load_users",
                                 python_callable= prepare_users_df)
    
    ratings_to_silver = PythonOperator(task_id= "load_ratings",
    ...
    # Gold tasks
    # -------------------
    ensure_staging = PythonOperator(task_id= 'check_create_stg_tables',
                                    python_callable= check_tables)
    
    dim_movies_to_gold = PythonOperator(task_id= 'load_movies_to_gold',
                                        python_callable= load_movie_df)

    dim_users_to_gold = PythonOperator(task_id= 'load_users_to_gold',
                                        python_callable= load_users_df)

    fct_ratings_to_gold = PythonOperator(task_id= 'load_ratings_to_gold',
                                        python_callable= load_ratings_df,
                                        op_kwargs= {'pipeline_start': pipeline_start})

    notify_failure = EmailOperator(task_id='notify_failure',
                                   to='marvee006@gmail.com',
                                   subject='DAG Failed: pipeline_flow', 
                                   html_content="""<h3>One or more tasks in pipeline_flow failed.</h3>""",
                                   trigger_rule='one_failed')

    notify_success = EmailOperator(task_id='notify_success', 
